...
set_property PACKAGE_PIN D14 [get_ports hc05_uart_rxd]  ## JB1 HC-05 TX

set_property IOSTANDARD LVCMOS33 [get_ports hc05_uart_txd]
set_property PACKAGE_PIN F16 [get_ports hc05_uart_txd]  ## JB2 HC-05 RX

##chat-gpt fix for DRC NSTD-1 / UCIO-1 error
##[DRC UCIO-1] Unconstrained Logical Port: 1 out of 5 logical ports have no user assigned specific location constraint (LOC). 
...
##or in extreme cases cause damage to the device or the components to which it is connected. To correct this violation, specify all pin locations. 
...
##NOTE: When using the Vivado Runs infrastructure (e.g. launch_runs Tcl command), add this command to a .tcl file and add that file as a pre-hook 
##for write_bitstream step for the implementation run.  Problem ports: reset_0.
set_property PACKAGE_PIN C2 [get_ports reset_0]
