...
fun AddUserCheckDBSplashScreen(navController: NavController, username: String, password: String, PIN: String)
{
    ...
    val isUserNameTaken = remember { mutableStateOf(false) } //Var to be updated based on DB check
    ...
    Column(
        ...
        LaunchedEffect(Unit){
            ...
            //Check Pseudo DB for username
            if (PseudoDatabase.isUsernameTaken(username)){
                isUserNameTaken.value = true //Var to be updated based on DB check
            }
            ...
            if (isUserNameTaken.value){
                ...
                //Add the new user
                PseudoDatabase.addUser(username, password, PIN)
                ...
                if (isUserNameTaken.value)
                {
                    ...
                    navController.navigate(Screen.AddUserScreen.route){
                        popUpTo(Screen.LoginScreen.route){
                            inclusive = false //Login should be on the stack so when the navigation back button is pressed it goes to the login screen
                        }
                    }
                }
