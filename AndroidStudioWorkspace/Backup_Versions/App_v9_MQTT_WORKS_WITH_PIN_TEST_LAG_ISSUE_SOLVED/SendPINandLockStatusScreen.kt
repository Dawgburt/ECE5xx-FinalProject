...
class MqttHelper {
    ...
    init {
        ...
        mqttConnectOptions = MqttConnectOptions().apply {
            isCleanSession = true
            isAutomaticReconnect = true // Enables auto-reconnect
        }

        mqttClient.setCallback(object : MqttCallback {
            override fun messageArrived(topic: String, message: MqttMessage) {
                Log.d("MQTT", "Message received on topic: $topic, Message: ${message.toString()}")
            }

            override fun connectionLost(cause: Throwable?) {
                Log.e("MQTT", "Connection lost, attempting to reconnect...", cause)
            }
    ...
    fun connect() {
        try {
            Log.d("MQTT", "Attempting to connect to MQTT broker...")
            mqttClient.connect(mqttConnectOptions)

            // Ensure connection is established
            if (mqttClient.isConnected) {
                Log.d("MQTT", "Connected to MQTT broker successfully!")
            } else {
    ...
    fun subscribeToLockStatus(onMessageReceived: (String) -> Unit) {
        try {
            if (!mqttClient.isConnected) {
                Log.e("MQTT", "Cannot subscribe, client is not connected")
                return
            }

            mqttClient.subscribe("ECE544x558/FP/App/RemoteKey/LockStatus")
            Log.d("MQTT", "Subscribed to ECE544x558/FP/App/RemoteKey/LockStatus")
    ...
    fun publishMessage(topic: String, message: String) {
        try {
            if (!mqttClient.isConnected) {
                Log.e("MQTT", "Client is not connected. Attempting to reconnect...")
            ...
            if (mqttClient.isConnected) {
                ...
                Log.e("MQTT", "Failed to reconnect, message not sent.")
            }
        } catch (e: MqttException) {
