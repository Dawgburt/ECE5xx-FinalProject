...
fun LoginCheckDBSplashScreen(navController: NavController, username: String, password: String) {
    ...
    LaunchedEffect(Unit) {
        ...
        if (isUserNameValid.value && isPasswordValid.value) {
            ...
            isError.value = false
            kotlinx.coroutines.delay(1000) // Let user read "Login Successful"
            feedbackMessage.value = "Connecting to MQTT Broker..." //The app doesn't hang, its just checking the MQTT broker.
